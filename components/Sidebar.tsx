...
const NavItem = ({ icon, label, active = false, collapsed = false, className = "" }: { icon: string, label: string, active?: boolean, collapsed?: boolean, className?: string }) => (
  <div className={`flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-all duration-200 group ${active ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-400 hover:text-white hover:bg-white/5'} ${className}`}>
    <span className={`material-symbols-outlined text-2xl transition-transform group-hover:scale-110 ${active ? 'fill-1' : ''}`}>{icon}</span>
    {!collapsed && <p className="text-sm font-bold whitespace-nowrap">{label}</p>}
  </div>
);
